...
class ShootingProblem
{
   public:
     ...
     */
    ShootingProblem(const VectorXs& x0, const std::vector<boost::shared_ptr<ActionModelAbstract> >& running_models,
     ...
     */
    double calc(const std::vector<VectorXs>& xs, const std::vector<VectorXs>& us);
     ...
     * @return The total cost value \f$l_{k}\f$
     */
    double calcDiff(const std::vector<VectorXs>& xs, const std::vector<VectorXs>& us);
     ...
     */
    friend std::ostream& operator<<(std::ostream& os, const ShootingProblem& problem);
